                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <h4>Daftar Jadwal UAS</h4>
                            <div class="col box-header text-right">
                                <a href="{{ route('tambah.trUAS') }}" class="btn btn-primary"><i class="fa fa-plus"></i
                                        &nbsp> Tambah</a>
                                <a href="{{ route('trUAS') }}" id="generate-uas"
                                    class="btn btn-success"><i class="fa fa-circle-nodes"></i &nbsp> Generate UAS</a>
                                <a href="{{ route('trUAS') }}" id="deletePUAS" class="btn btn-danger dis disabled"><i
                                                    ...
                                                    <div class="custom-checkbox custom-control">
                                                        <label class="checkbox-inline">
                                                            <input type="checkbox" class="check"
                                                                value="{{ $j->id_trjadwal }}" name="id_truas" id="test-cb">
                                                            <span class="checkmark"></span>
                                                        </label>
                                                    </div>
                                                </td>
...
@push('scripts')
    ...
    <script>
        document.getElementById('deletePUAS').onclick = function() {
        var idnya = [];
        var marked = document.getElementsByName('id_truas');
        ...
        var confirmDialog = confirm(
            "Apakah Anda yakin ingin menghapus seluruh data penjadwalan UAS yang dipilih?");
        ...
        if (confirmDialog) {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    iziToast.success({
                        title: 'Berhasil',
                        message: 'Data penjadwalan UAS berhasil dihapus.',
                        position: 'topRight'
                    });
                },
    ...
    <script>
    document.getElementById("generate-uas").addEventListener("click", function() {
        ...
        $.ajax({
            type: 'POST',
            url: '{{ route('generate.uas') }}', // Ganti dengan URL yang sesuai
            data: data,
            success: function(data) {
                console.log(data);
                iziToast.success({
                    title: 'Berhasil',
                    message: 'Data penjadwalan UAS berhasil ditambahkan.',
                    position: 'topRight'
                });
            },
             error: function(xhr, textStatus, errorThrown) {
                ...
                if (errorMessage) {
                    ...
                    iziToast.error({
                        title: 'Gagal',
                        message: 'Pengawas tidak memenuhi kebutuhan, penjadwalan UAS tidak dilakukan secara menyeluruh.',
                        position: 'topRight'
                    });
                }
            },
